...
def calendar_contact_card(lead_id):
    ...
    with adv_cols[1]:
        if current_pipeline < 1:
            if st.button("📞 Discovery", key=f"card_to_disc_{lead_id}", use_container_width=True):
                st.session_state[f"show_disc_picker_{lead_id}"] = True
    ...
    if st.session_state.get(f"show_disc_picker_{lead_id}"):
        with st.container(border=True):
            ...
            with dc1:
                disc_date = st.date_input("Call Date", value=datetime.now() + timedelta(days=2), key=f"disc_date_{lead_id}")
            with dc2:
                disc_time = st.time_input("Call Time", value=datetime.now().replace(hour=14, minute=0).time(), key=f"disc_time_{lead_id}")
    ...
    if not st.session_state.leads.empty:
        ...
        if "Social" in df_results.columns:
            def _social_icons(social_dict):
                ...
                if social_dict.get("youtube"): icons.append("🎥")
                if social_dict.get("tiktok"): icons.append("🎵")
...
""")
        ...
        event = st.dataframe(
                ...
                column_config={
                    ...
                    "Socials": st.column_config.TextColumn("Socials", width="small", help="🔗=LinkedIn 📘=Facebook 📸=Instagram 🐦=Twitter 🎥=YouTube 🎵=TikTok"),
        ...
        if lead:
            ...
            if stage == "1. Connect":
                ...
                with st.expander(f"{stage1_icon} **Stage 1: Find the Company & Directors**", expanded=not stage1_done):
                    ...
                    st.markdown("**Search these sources to find who runs this company:**")
                    ...
                    with s1_c1:
                        if is_uk:
                            ...
                            if ch_num:
                                ...
                                st.markdown(f"🏛️ **[Companies House →]({ch_url})**")
                            else:
                                ch_search = f"https://find-and-update.company-information.service.gov.uk/search?q={biz_name_encoded}"
                                st.markdown(f"🏛️ **[Companies House →]({ch_search})**")
                            st.caption("Directors, owners & filings (UK)")
                        else:
                            oc_search = f"https://opencorporates.com/companies?q={biz_name_encoded}"
                            st.markdown(f"🌍 **[OpenCorporates →]({oc_search})**")
                            st.caption("Directors & filings (Worldwide)")
                    with s1_c2:
                        li_company = f"https://www.linkedin.com/search/results/companies/?keywords={biz_name_encoded}"
                        st.markdown(f"🏢 **[LinkedIn Company →]({li_company})**")
                        st.caption("Company page → People tab")
                    with s1_c3:
                ...
                if stage1_done:
                    ...
                    with st.expander(f"{stage2_icon} **Stage 2: Find {clean_contact}'s Profile**", expanded=not stage2_done):
                        ...
                        with s2_c1:
                            li_person = f"https://www.linkedin.com/search/results/people/?keywords={contact_encoded}"
                            st.markdown(f"👔 **[Search LinkedIn for {clean_contact} →]({li_person})**")
                            
                            li_xray = f"https://www.linkedin.com/search/results/people/?keywords={biz_name_encoded}&origin=FACETED_SEARCH"
